...
<img width="1106" height="552" alt="UML" src="https://github.com/user-attachments/assets/ebae1335-5791-405d-ae8a-e6a34756424b" />
...
## 6. Проектирование реляционной структуры хранения данных (ERD)

Для фиксации участников и движений денежных средств спроектирована реляционная структура базы данных PostgreSQL (модель «Один ко многим»). Каждая транзакция СБП строго привязана к отправителю, а защита от дублирования платежей обеспечена строгим контролем уникальности UUID-ключей идемпотентности.

![Диаграмма базы данных ERD](images/db11.png)

---

## 7. Проектирование логики работы с БД (SQL-скрипты)

### 7.1. Оптимизированная проверка баланса отправителя (перед холдированием средств)
```sql
SELECT EXISTS (
    SELECT 1 
    FROM accounts 
...
### 7.2. Обновление статуса транзакции до успешного после подтверждения от ОПКЦ СБП
```sql
UPDATE transactions 
SET status = 'SUCCESS' 
WHERE id = 555;
...
## 8. Верификация и тестирование API контракта (Postman)
...
<img width="1198" height="781" alt="post" src="https://github.com/user-attachments/assets/0cc21c68-307a-475a-aa2d-f93f7502601b" />
