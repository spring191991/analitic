...
### 3. Проектирование схемы маппинга данных для параметрической номенклатуры
В рамках интеграции каталога MODX провела детальный анализ структуры входящих данных от поставщиков. Лично определила соответствие полей исходного файла текущим свойствам товаров в базе данных сайта, спроектировала схему маппинга (сопоставления) данных и согласовала с менеджментом добавление новых динамических атрибутов для расширения характеристик номенклатуры.

### 4. Проектирование интеграции со сложной структурой связей (CMS Data Mapping)
В рамках параллельного проекта на 1С-Битрикс стандартные модули импорта не справлялись со спецификой архитектуры СУБД, где данные о товарах жестко разделены между двумя связанными сущностями — инфоблоком «Карточки товаров» и инфоблоком «Каталог». 

**Решение:** Выявила логику взаимного соответствия полей и спроектировала кастомную схему распределения данных. Это обеспечило корректное асинхронное связывание данных в СУБД между обоими инфоблоками без нарушения целостности базы.

### 5. Оптимизация архитектуры: Асинхронное выполнение (Cron-расписание)
