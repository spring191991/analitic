...
## 5. Проектирование структуры хранения данных и каскадного кэширования (ERD)

Для обеспечения высокой производительности системы (SLA 99.99%) и минимизации нагрузки на дисковую подсистему спроектирована гибридная структура данных:
1. **Реляционная СУБД PostgreSQL:** Хранит партиционированную историю транзакций с точным типом данных `numeric` для сумм и `timestamp` для фиксации времени операций.
2. **In-Memory хранилище Redis:** Выступает в роли быстрого Key-Value кэша, фиксируя актуальный текущий баланс клиента по первичному ключу `client_id`. 

Это позволяет оконной функции SQL мгновенно рассчитывать нарастающий итог баланса в обратном порядке только для 20 записей на выбранной странице пагинации.

![Структура баз данных каскада](images/db22.png)
...
## 7. Верификация и тестирование контракта API (Postman Mock Server)
...
## 🛠️ Автоматизация и Мониторинг (Python)

Для прототипирования системы сбора метрик и валидации контрактов API высоконагруженного сервиса выписок разработан легковесный скрипт автоматизации. Он позволяет анализировать "сырые" логи кэширующего слоя и API-ответов.

* **Расположение компонента:** [`/scripts/log_analyzer.py`](./scripts/log_analyzer.py)
* **Тестовый датасет:** [`/scripts/api_responses.json`](./scripts/api_responses.json)

### Реализованный функционал:
1. **Анализ DTO-структур:** Скрипт парсит JSON-пакеты ответов от микросервисов и обрабатывает их в цикле.
2. **Фильтрация и изоляция ошибок:** Автоматически выявляются и подсчитываются запросы со статусом `FAILED`. Реализована безопасная обработка данных с защитой от `None/Null` значений.
3. **Финансовая агрегация:** Производится динамический расчет суммарного баланса по всем успешным операциям выгрузки (`COMPLETED`).

Этот скрипт применяется для оперативной проверки целостности структуры данных (API Contract Validation) на этапе интеграционного тестирования.
